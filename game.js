class RustEchkaGame {
    ...
    connectToServer() {
        ...
        this.socket.on('playerResources', (data) => {
            if (data.playerId === this.myPlayer.id) {
                ...
                this.otherPlayers[data.playerId].wood = data.wood;
                this.otherPlayers[data.playerId].stone = data.stone;
            }
    ...
    updatePlayersList() {
        ...
        list.innerHTML = '<div style="color: #4ecdc4; margin-bottom: 10px; font-weight: bold;">👥 Игроки онлайн:</div>';
        
        // Наш игрок
        const myPlayerItem = document.createElement('div');
        myPlayerItem.className = 'player-item';
        myPlayerItem.innerHTML = `
            <span class="player-color" style="background: ${this.myPlayer.color}"></span>
            <span style="color: ${this.myPlayer.color}; font-weight: bold;">${this.myPlayer.name}</span>
            <span style="margin-left: auto;">🌳${this.myPlayer.wood} ⛰️${this.myPlayer.stone}</span>
        `;
        ...
        Object.values(this.otherPlayers).forEach(player => {
            ...
            playerItem.innerHTML = `
                ...
                <span style="margin-left: auto;">🌳${player.wood} ⛰️${player.stone}</span>
            `;
    ...
    render() {
        ...
        this.resources.stones.forEach(stone => {
            if (stone.stone > 0) {
                ...
                this.ctx.font = '12px Arial';
                this.ctx.textAlign = 'center';
                this.ctx.fillText(`⛰️${stone.stone}`, stone.x, stone.y + 35);
            }
        });
